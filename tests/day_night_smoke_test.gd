...
func _run() -> void:
	...
	var danger_before_flashlight := float(main_scene.get("raid_danger"))
	main_scene.call("_update_raid_danger", 1.0)
	assert(float(main_scene.get("raid_danger")) > danger_before_flashlight)
