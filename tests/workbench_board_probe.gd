...
#
# 컷(res://test-output):
#   ① workbench_board_enhance_landscape   강화 탭 기본(AK 선택, 1280x720)
...
func _run() -> void:
	...
	_check(layer.find_child("WorkbenchTab_enhance", true, false) != null and layer.find_child("WorkbenchTab_craft", true, false) != null and layer.find_child("WorkbenchTab_supply", true, false) != null, "탭 3개(강화/제작/개조·보급)")
	var rows := 0
	...
	_check(layer.find_child("WorkbenchMaterialsPanel", true, false) != null, "재료 패널 존재")
	...
	_check(primary.text.begins_with("돌파"), "주 버튼이 붉은 [돌파 · 장인에게] (실제: %s)" % primary.text)
	_check(max_button.disabled, "[가능한 만큼] 비활성(관문)")
	_log_modal_rect("landscape-gate")
	await _capture("workbench_board_gate")

	# 돌파 호출 — 인장 1 + 정밀 기어(지금 3) → 비용에 따라 실패/성공 모두 사유가 토스트로.
	print("[4] [돌파] 호출")
	...
	game_state.call("add_mod_component", "precision_gear", int(gate_cost.get("precision_gear", 0)))
	game_state.call("add_mod_component", "military_alloy", int(gate_cost.get("military_alloy", 0)))
	...
	_check(recipe_rows.size() == 16, "제작 탭 16종 (실제: %d)" % recipe_rows.size())
	...
	(layer.find_child("WorkbenchTab_supply", true, false) as Button).pressed.emit()
	await _sleep(0.4)
	layer = root.find_child("WorkbenchUILayer", true, false) as CanvasLayer
	_check(layer.find_child("WorkbenchRecipeRow_workbench_upgrade", true, false) != null, "개조·보급 탭 하단에 작업대 확장 행")
