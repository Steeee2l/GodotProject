...
func _build_environment() -> void:
	...
	environment.background_color = Color("#11191c")
	environment.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	environment.ambient_light_color = Color("#748087")
	environment.ambient_light_energy = 0.58
	environment.tonemap_mode = Environment.TONE_MAPPER_FILMIC
	environment.fog_enabled = true
	environment.fog_light_color = Color("#647176")
	...
	moon_light.light_color = Color("#aeb9bc")
	...
	_add_bridge_rail(6.15)
	_add_vehicle("res://assets/opening/opening_wrecked_taxi_v1.png", Vector3(-3.5, 0.84, 8.0), 0.0036, 12.0, Vector3(4.1, 1.1, 1.8))
	_add_vehicle("res://assets/opening/opening_wrecked_truck_v1.png", Vector3(2.4, 0.95, -10.5), 0.0040, -8.0, Vector3(5.0, 1.3, 2.1))
	_add_vehicle("res://assets/opening/opening_wrecked_taxi_v1.png", Vector3(3.7, 0.82, 20.0), 0.0032, -18.0, Vector3(3.7, 1.0, 1.7))
	_add_vehicle("res://assets/opening/opening_wrecked_bus_v1.png", Vector3(-1.8, 1.0, -23.5), 0.0044, 4.0, Vector3(5.8, 1.3, 2.2))
	_add_fire(Vector3(-3.2, 0.35, 7.7), 1.0)
	_add_fire(Vector3(2.9, 0.4, -10.8), 1.2)
	_add_fire(Vector3(-5.5, 0.3, -18.0), 0.8)
	_add_mist()
...
func _add_bridge_rail(x_position: float) -> void:
	...
	mesh.size = Vector3(0.28, 0.72, 72.0)
	mesh.material = material
	var body := StaticBody3D.new()
...
func _build_camera() -> void:
	...
	camera_rig.position = Vector3(0, 0, 24)
...
func _build_player() -> void:
	...
	player.position = Vector3(0, 0.78, 25.0)
...
func _build_extraction() -> void:
	...
	sewer_exit.position = Vector3(0, 0.08, -28.0)
...
func _build_hud() -> void:
	...
	dim.color = Color(0.02, 0.025, 0.028, 0.08)
...
func _build_dialogue_ui(hud: CanvasLayer) -> void:
	...
	continue_label.text = "클릭 또는 SPACE"
...
func _build_status_ui(hud: CanvasLayer) -> void:
	...
	health_panel.size = Vector2(244, 54)
	...
	health_margin.add_theme_constant_override("margin_top", 10)
	health_margin.add_theme_constant_override("margin_bottom", 10)
	health_panel.add_child(health_margin)
	health_bar = ProgressBar.new()
	health_bar.max_value = PLAYER_MAX_HEALTH
	health_bar.value = player_health
	health_bar.show_percentage = false
	health_bar.custom_minimum_size = Vector2(216, 30)
	...
	health_margin.add_child(health_bar)
	var ammo_panel := PanelContainer.new()
	ammo_panel.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT)
	ammo_panel.position = Vector2(-286, -142)
	ammo_panel.size = Vector2(252, 84)
	ammo_panel.add_theme_stylebox_override("panel", _panel_style(Color(0.015, 0.02, 0.022, 0.9), Color("#a98e4d"), 2, 5))
	hud.add_child(ammo_panel)
	var ammo_margin := MarginContainer.new()
	ammo_margin.add_theme_constant_override("margin_left", 18)
	ammo_margin.add_theme_constant_override("margin_right", 18)
	ammo_margin.add_theme_constant_override("margin_top", 10)
	...
	ammo_label.add_theme_color_override("font_color", Color("#f0d27d"))
	ammo_margin.add_child(ammo_label)
...
func _build_interaction_ui(hud: CanvasLayer) -> void:
	...
	interaction_label.text = "[F]  하수구로 내려가기"
...
func _spawn_cinematic_enemies() -> void:
	var setups := [
		{"start": Vector3(-1.8, 0.78, -9.0), "end": Vector3(-1.8, 0.78, 2.0), "weapon": "m1911"},
		{"start": Vector3(1.7, 0.78, -12.0), "end": Vector3(1.7, 0.78, -0.5), "weapon": "mp5"},
	]
	for setup in setups:
		var enemy := CharacterBody3D.new()
		enemy.set_script(ENEMY_SCRIPT)
		enemy.call("configure", "pistol", player, {}, 0.0, str(setup["weapon"]))
		enemy.position = setup["start"]
		enemy.set_meta("cinematic_end", setup["end"])
...
func _start_camera_reveal() -> void:
	...
	camera_tween.tween_property(camera_rig, "position", Vector3(0, 0, 2.0), 1.75)
	camera_tween.parallel().tween_property(camera, "size", 20.0, 1.75)
	for enemy in enemies:
		if not is_instance_valid(enemy):
			continue
		var end_position: Vector3 = enemy.get_meta("cinematic_end", enemy.position)
		var enemy_tween := create_tween()
		enemy_tween.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		enemy_tween.tween_property(enemy, "position", end_position, 1.55)
...
func _update_player_gameplay(delta: float) -> void:
	...
	player.position.z = clampf(player.position.z, -29.0, 29.0)
	...
	if phase == "tutorial_combat" and fire_held and aim_held:
		_try_fire()
...
func _spawn_roll_afterimages() -> void:
	for delay in [0.05, 0.11, 0.17]:
		get_tree().create_timer(delay).timeout.connect(func() -> void:
			if not is_instance_valid(player_sprite) or restarting:
				return
...
func _start_tutorial_combat() -> void:
	...
	_show_objective(
		"교량 돌파",
		"오른쪽 버튼으로 조준하고 왼쪽 버튼으로 앞의 적들을 소탕하세요.",
		"남은 적 %d" % enemies_remaining
	)
...
func _update_weapon_visual() -> void:
	...
	if aim_reticle.visible:
		aim_reticle.position = get_viewport().get_mouse_position() - aim_reticle.size * 0.5
...
func _update_hud() -> void:
	if health_bar:
		health_bar.value = player_health
	if ammo_label:
		ammo_label.text = "AK-47  ·  7.62mm\n%d / %d" % [magazine_ammo, reserve_ammo]
		if reloading:
			ammo_label.text += "  ·  재장전 %.1f" % reload_remaining
