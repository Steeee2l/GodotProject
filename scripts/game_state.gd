...
func get_pending_shelter_story_event() -> Dictionary:
	...
	if not saja_intro_seen:
		return {
			...
			"speaker": "사자",
			"title": "쉘터 관리자",
			# 쉘터에서 처음 만나는 인물 — 첫 만남의 일: 긴장 풀기 → 쉘터 소개 →
			# 오프닝의 라디오 신호를 첫 미션(종로 '신호를 보낸 것')으로 잇기 →
			# 규칙 한 줄 → 다음 행동. '이름을 적는다'는 다정해 보이지만 반전
			# (명단을 받아 적은 손)의 복선이다. 겉은 험악, 말은 꼼꼼하고 예의 바르게.
			"lines": [
				"총은 내려도 됩니다. 다리를 건너왔습니까? 요즘 그걸 해내는 고양이는 드뭅니다.",
				"여기는 쉘터. 갈 곳 없는 고양이들이 모여 사는 곳입니다. 나는 사자, 장부를 맡고 있습니다.",
				"라디오를 듣고 왔다고요? …이상하군요. 우리는 송신기를 쓰지 않습니다.",
				"그 신호가 어디서 나왔는지 — 나라면 종로 쪽을 알아보겠습니다. 당신 총이면 갈 수 있겠죠.",
				"규칙은 하나입니다. 밖에서 가져온 건 값을 쳐 받고, 안에서는 아무도 굶지 않습니다.",
				"이름을 적어 두겠습니다. …됐습니다. 오늘부터 이 쉘터 식구입니다.",
				"우선 쉬세요. 여기선 상처가 빨리 아뭅니다. 일은 내일부터 밀려 있을 테니까.",
			],
		}
