...
const SAJA_FACILITY_CONTRACTS: Array[Dictionary] = [
	...
	},
	{
		...
		"accept_dialogue": [
			"버려진 차나 설비를 그냥 지나치기는 아깝지 않냐.",
			"두 개만 분해해 봐라. 뭐가 쓸 만한지는 몸으로 배우는 게 제일 빠르다.",
		],
		"complete_dialogue": [
			"부품 상태가 좋네. 작업대 애들이 신났다.",
	...
	},
	{
		...
		"accept_dialogue": [
			"침상이 하나 빈다. 빈 침대를 보면 마음이 영 안 좋다.",
			"밖에 아직 혼자 버티는 애들이 있다. 하나만 데려와라.",
		],
		"complete_dialogue": [
...
const SAJA_CHATTER := {
	"early": [
		...
		{"title": "네 몫", "lines": [
			"오늘 배급은 국물 있는 거다. 네 몫은 내가 따로 덜어 놨다.",
			"왜 그렇게 보냐. 밖에 나가는 놈은 너 하나다. 네 몫이 큰 게 당연하다.",
		]},
		{"title": "걸음소리", "lines": [
			"망루 애들이 그러더라. 네 걸음소리는 멀리서도 알아듣겠다고.",
			"조용히 걷는 것도 배워 둬라. 종로에 있는 놈들도 귀가 있다.",
		]},
		{"title": "그 방송", "lines": [
			"그 방송 말인데. 나는 그 소리를 몇 년째 듣고 있다. 이제는 지겹다.",
			"너한테는 아직 새 소리겠지. 종로에서 답을 찾으면 나한테도 말해 줘라.",
		]},
		{"title": "다친 데", "lines": [
			"지난번에 다친 데는 어떠냐. 내가 그런 건 다 기억한다. 버릇이야.",
			"종로에서는 다친 데부터 살펴라. 총알은 그다음이다.",
		]},
		{"title": "바깥", "lines": [
			"애들이 자꾸 묻는다. 밖은 어떠냐고. 나는 모른다고 대답한다.",
			"나는 밖에 나가 본 지 너무 오래됐다. 네 얘기가 애들한테는 바깥 소식이다.",
		]},
	],
	"mid": [
		{"title": "명단", "lines": [
			"명단 얘기는 애들한테 들었다. 이름이 스물일곱 개라며.",
			"그중에 네가 아는 이름은 없었냐. 없으면 됐다. 밥 먹어라.",
		]},
		{"title": "생선 대가리", "lines": [
			"남대문 시장은 옛날에 나도 자주 갔다.",
			"거기 생선 대가리를 공짜로 주던 할머니가 있었다.",
			"그 할머니가 어떻게 됐는지는 모른다. 시장에 가면 좀 살펴봐라.",
		]},
		{"title": "기록 상자", "lines": [
			"네가 가져온 기록은 내가 상자에 넣어 뒀다. 잃어버릴 일은 없다.",
			"보고 싶으면 언제든 말해라. 내가 꺼내 준다.",
		]},
		{"title": "잠", "lines": [
			"요즘 잠을 설친다. 쉘터가 커지니까 지킬 게 많아졌다.",
			"너도 자는 얼굴이 편하지 않더라. 밖에서 본 게 많은 거겠지.",
			"오늘은 푹 자라.",
		]},
		{"title": "주홍", "lines": [
			"주홍이 다녀갔다며. 걔가 나를 싫어하는 건 나도 안다.",
			"걔는 하수구에서 오래 혼자 살았다. 그래서 사람을 잘 안 믿는다.",
			"그래도 네 뒤는 잘 봐 줄 거다. 걔 말은 잘 들어라.",
		]},
		{"title": "을지로", "lines": [
			"을지로 밑에는 옛날 관공서 창고가 많았다. 서류 상자가 굴러다닐 거다.",
			"쓸 만한 기록이 보이면 챙겨 와라. 무거우면 놓고 와도 된다.",
		]},
	],
	"late": [
		{"title": "달라진 눈", "lines": [
		...
		{"title": "어두운 데", "lines": [
			"을지로 밑까지 내려갔다 왔다며. 거기 어두웠지.",
			"나는 어두운 데가 싫다. 그래서 여기 등은 밤에도 안 끈다.",
		]},
		{"title": "밥은 밥", "lines": [
			"네가 밖에서 무슨 얘기를 들었는지 나는 안 묻는다.",
			"그래도 밥은 먹어라. 식기 전에 먹어. 그건 안 변한다.",
		]},
		{"title": "그날", "lines": [
			"사람들이 사라진 날 얘기다. 나는 그날 여기서 문을 잠그고 있었다.",
			"애들이 못 나가게 잠갔다. 그게 그날 내가 한 일이다.",
		]},
		{"title": "남산", "lines": [
			"남산에 올라갈 거지. 거기는 길이 가파르다. 옷 든든히 입고 물 챙겨라.",
...
func get_pending_shelter_story_event() -> Dictionary:
	...
	if (
		...
		return {
			...
			"title": "쇳내",
			"lines": [
				"가방에서 쇳내가 난다. 스프링에 렌즈까지. 좋은 걸 주웠네.",
				"그 부품은 작업대에서 총에 달아라. 하나 달 때마다 총이 조금씩 좋아진다.",
				"총은 좋은 걸 들어. 밖에 나가는 건 너 하나다. 그게 늘 마음에 걸린다.",
			],
		}
	...
	for zone_id in MAIN_MISSION_CATALOG.ZONE_ORDER:
		...
		if next_zone_id.is_empty():
			...
			closing_lines.append("다음은 %s겠구나. 가지 말라고 해도 너는 들을 놈이 아니지." % next_name)
