...
const SAJA_FACILITY_CONTRACTS: Array[Dictionary] = [
	{
		...
		"lore": "사자는 뭐든 먼저 숫자로 적는다. 부품 세 개, 주민 한 명, 통조림 여덟 개. 왜 필요한지는 적지 않는다.",
	},
	{
		"id": "street_patrol",
		"title": "운반로 정리",
	...
	},
	{
		...
		"lore": "뭐든 원본과 대조한다. 대조할 수 있다는 건 원본을 갖고 있다는 뜻이다.",
	},
	{
		"id": "salvage_cipher",
		"title": "설비 분해 두 건",
		...
		"lore": "창고에 라벨 없는 상자가 몇 개 있다. 목록에도 없다. 물어보면 규정 얘기를 한다. 상자 하나에서 사람 옷 냄새가 난다.",
	},
	{
		"id": "rescue_route",
		"title": "빈 침상 한 자리",
		...
		"lore": "새 주민이 올 때마다 이름을 적는다. 글씨가 꾹꾹 눌려 있다. 오래 그렇게 써 온 손이다.",
	},
	{
		"id": "field_operation",
		"title": "현장 작전 한 건",
...
const SAJA_CHATTER := {
	"early": [
		{"title": "흙먼지", "lines": [
			"얼굴에 흙 묻었다. 이리 와 봐. …됐다.",
			"종로에서 무슨 일 있어도 이 문만 두드려. 내가 연다.",
		]},
		{"title": "네 몫", "lines": [
			"오늘은 국물 있는 거다. 네 몫 따로 덜어 놨어.",
			"왜 봐. 밖에 나가는 놈이 너 하난데 당연하지.",
		]},
		{"title": "걸음소리", "lines": [
			"망루 애들이 그러더라. 네 발소리는 멀리서도 알겠다고.",
			"조용히 걷는 것도 배워. 종로 놈들도 귀 있다.",
		]},
		{"title": "그 방송", "lines": [
			"그 방송… 나는 몇 년째 듣는다. 이제 지겨워.",
			"너한텐 아직 새 소리겠지. 답 찾으면 나한테도 말해 줘.",
		]},
		{"title": "다친 데", "lines": [
			"지난번 다친 데는.",
			"…됐다. 총알보다 그게 먼저다.",
		...
		{"title": "바깥", "lines": [
			"애들이 자꾸 묻는다. 밖은 어떠냐고.",
			"나는 모른다고 한다. 나간 지 너무 오래됐어. 네 얘기가 걔들한텐 바깥이다.",
		]},
	],
	"mid": [
		{"title": "명단", "lines": [
		...
		{"title": "생선 대가리", "lines": [
			"남대문, 옛날에 나도 자주 갔다. 생선 대가리 공짜로 주던 할머니가 있었어.",
			"어떻게 됐는지는 몰라. 가면 좀 봐 줘.",
		]},
		{"title": "기록 상자", "lines": [
			"네가 가져온 건 상자에 넣어 뒀다. 잃어버릴 일 없어.",
			"보고 싶으면 말해. 꺼내 준다.",
		]},
		{"title": "잠", "lines": [
			"요즘 잠을 설친다. 지킬 게 많아져서.",
			"너도 자는 얼굴이 안 편하더라. …오늘은 좀 자.",
		]},
		{"title": "주홍", "lines": [
			"주홍이 다녀갔다며. 걔가 나 싫어하는 거 안다.",
			"하수구에서 오래 혼자 살았어. 그래서 아무도 안 믿어. 그래도 네 뒤는 봐 줄 거다.",
		]},
		{"title": "을지로", "lines": [
			"을지로 밑에 옛날 관공서 창고가 많다. 서류 상자가 굴러다닐 거야.",
			"무거우면 놓고 와. 종이는 안 도망간다.",
		]},
	],
	"late": [
		{"title": "달라진 눈", "lines": [
		...
		{"title": "어두운 데", "lines": [
			"을지로 밑에 갔다 왔다며. 어두웠지.",
			"나는 어두운 데가 싫다. 그래서 여기 등은 밤에도 안 꺼.",
		]},
		{"title": "밥은 밥", "lines": [
			"밖에서 무슨 얘길 들었는지 안 묻는다.",
			"…그래도 먹어라. 식는다.",
		]},
		{"title": "그날", "lines": [
			"그날 밤 나는 여기서 문을 잠그고 있었다.",
			"애들 못 나가게. …그게 그날 내가 한 일이다.",
		]},
		{"title": "남산", "lines": [
			"남산이지.",
			"거기 올라가면 나 보기 싫어질 거다. 그래도 가.",
		]},
		{"title": "사나운 꿈", "lines": [
			"어젯밤 꿈에 네가 나왔다.",
			"…말 안 한다. 그냥 돌아와라.",
...
func get_pending_shelter_story_event() -> Dictionary:
	...
	if not saja_intro_seen:
		return {
			...
			"lines": [
				# 문장 규칙(2026-08-30 유저 확정): 한 문장에 사실 하나, 주어·목적어를
				# 빼먹지 않는다. "다리 망루가 한 시간 전부터 봤다" 같은 압축문은 금지.
				"총 내려.",
				"한 시간 전부터 봤다. 다리에서 세 번 넘어지더라.",
				"됐어, 누군지는 안 물어. 라디오 듣고 왔겠지. 여기 다 그래.",
				"나는 사자다. 살림 보는 놈이고. 규칙은 하나야. 밖에서 가져온 건 값을 쳐준다. 안에서는 아무도 안 굶는다.",
				"밥부터. 얘기는 그다음이다.",
				"이름.",
				"{name}. …적어 놔야 안 잊어.",
				"오늘은 자라. 종로는 내일도 거기 있다.",
			],
		}
	...
	if survived_return_count >= 1 and not saja_second_run_intro_seen:
		return {
			...
			"lines": [
				"왔냐.",
				"…밥부터. 데워 놨다.",
				"종로역 말이다. 그날 밤 거기만 끝까지 불이 켜져 있었어.",
				"뭘 봤든 나한테 말해. 혼자 들고 있으면 무거워.",
				"철근이 너 보더라. 무뚝뚝해도 나쁜 놈은 아니야. 시간 나면 가 봐.",
			],
		}
	...
	if (
		...
		return {
			...
			"title": "쇳내",
			"lines": [
				"가방에서 쇳내 난다. 스프링에 렌즈까지.",
				"작업대에 갖다 대. 하나 달 때마다 총이 조금씩 나아진다.",
				"…밖에 나가는 건 너 하나잖아. 그게 자꾸 걸려.",
			],
		}
	...
	if total_boss_kills > saja_seen_boss_kills:
		return {
			...
			"title": "바깥 소문",
			"lines": [
				"밖이 시끄러웠다며. 애들이 네 얘기만 한다.",
				"무리하지 마. 그런 놈 자리는 금방 다른 놈이 채워.",
				"…오늘은 자랑해도 된다. 한 그릇 더 퍼 줄게.",
			],
		}
	...
	for zone_id in MAIN_MISSION_CATALOG.ZONE_ORDER:
		...
		var closing_lines: Array[String] = [
			"%s 갔다 왔다며. 앉아." % zone_name,
			"네가 가져온 건 내가 잘 두고 있다. 다 이어 보면 뭐가 보일 거야.",
		]
		if next_zone_id.is_empty():
			closing_lines.append("이제 남산이지.")
			closing_lines.append("…끝을 보고 와. 나는 여기 있는다.")
		else:
			var next_name := str(
				(RAID_ZONES.get(next_zone_id, {}) as Dictionary).get("name", "다음 구역")
			)
			closing_lines.append("다음은 %s겠지. 말려도 안 들을 거고." % next_name)
			...
			closing_lines.append("준비되면 그쪽 길도 열린다. 서두를 거 없어.")
		return {
	...
	if subway_story_stage > saja_seen_subway_stage:
		return {
			...
			"title": "지하의 소리",
			"lines": [
				"지하에서 신호가 다시 잡힌단다. 기계는 저 혼자 안 켜져.",
				"…내려갈 거지. 조심해라.",
				"주홍이 먼저 갔을 거다. 만나면 내 안부나 전해. 안 믿겠지만.",
			],
		}
